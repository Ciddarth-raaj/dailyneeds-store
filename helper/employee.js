...
const EmployeeHelper = {
  getEmployee: (filter) =>
    new Promise(function (resolve, reject) {
      ...
      if (filter?.store_ids) {
        filter.store_ids.forEach((id, index) => {
          queryParams.append(`store_ids[${index}]`, id);
        });
      }

      if (filter?.designation_ids) {
        filter.designation_ids.forEach((id, index) => {
          queryParams.append(`designation_ids[${index}]`, id);
        });
      }
