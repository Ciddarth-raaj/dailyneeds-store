...
export default class NewJoiner extends React.Component {
  ...
  //     EmployeeHelper.getStoreById(store_id)
  //       .then((data) => {
  //         let date = [];
  //         let count = [];
  //         let detailsMapper = {};
  //         let updatedCountArr = [];
  //         data.map((detail) => {
  //           detailsMapper[moment(detail.created_at).format("MMMM")] =
  //             detail.store_count;
  //         });
  //         MONTH.map((el1) => {
  //           var temp = {};
                  ...
                  >
                    {check_store === "" ? (
                      <div className={styles.dropdown}>
                        ...
                        />
                        <div
                          className={styles.newDropdowncontent}
                          style={
                            hoverElement === false
                              ? { color: "black" }
                              : { display: "none" }
                        ...
                        >
                          {store_data
                            .filter(
                              ({ outlet_name }) =>
                                outlet_name.indexOf(store.toLowerCase()) > -1
                            )
                            .map((m) => (
                              <a
                                onClick={() =>
                                  this.setState({
                                    store_name: m.outlet_name,
                                    store_id: m.outlet_id,
                                    hoverElement: true,
                                  })
                                }
                              >
                          ...
                          {anniversary.length !== 0 &&
                            anniversary.map((m) => (
                              <div className={styles.anniversaryContent}>
                                ...
                                <p className={styles.birthdayContent}>
                                  {moment(m.date_of_joining).format(
                                    "DD MMMM YYYY"
                                  )}
                                </p>
                              </div>
